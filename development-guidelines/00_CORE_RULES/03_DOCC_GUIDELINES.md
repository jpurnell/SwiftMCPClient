# DocC Documentation Guidelines for SwiftMCPClient
...
- [ ] Includes all necessary imports (e.g., `import SwiftMCPClient`)
...
swift package --disable-sandbox preview-documentation --target SwiftMCPClient
...
Structure:
```
Sources/SwiftMCPClient/SwiftMCPClient.docc/
├── SwiftMCPClient.md              # Landing page
...
SwiftMCPClient provides tools for:
- [Core capability 1]
- [Core capability 2]
- [Core capability 3]
- [Core capability 4]
...
or [use case C], SwiftMCPClient offers a robust,
...
`GettingStarted.md`:
```markdown
# Getting Started with SwiftMCPClient

Learn the basics of using SwiftMCPClient for [primary use case].

## Overview

This tutorial covers:
- Installing SwiftMCPClient
- Creating models and data stores
- Basic calculations
- Building a simple workflow

### Add SwiftMCPClient to Your Project

Add the dependency to your `Package.swift`:

```swift
dependencies: [
    .package(url: "https://github.com/username/SwiftMCPClient", from: "2.0.0")
]
```
...
Understand and apply the fundamental calculations in SwiftMCPClient.

## Overview

SwiftMCPClient is built around a set of core concepts:
...
> 3. ✅ Added to the `SwiftMCPClient.md` landing page
...
SwiftMCPClient provides a comprehensive framework...
...
cd Sources/SwiftMCPClient/SwiftMCPClient.docc
...
**Why it happens**: Creating a `.md` file in the `.docc` directory is not enough. The article must be explicitly referenced in the main landing page (`SwiftMCPClient.md`) to appear in navigation.

**Solution**: After creating any new tutorial or guide, immediately add it to the `SwiftMCPClient.md` landing page in the appropriate section.

**Steps**:
1. Create your tutorial file (e.g., `NewFeatureGuide.md`)
2. Open `Sources/SwiftMCPClient/SwiftMCPClient.docc/SwiftMCPClient.md`
...
- [ ] Added to appropriate section in `SwiftMCPClient.md`
...
- [ ] Added to `SwiftMCPClient.md` landing page in appropriate `## Topics` section
...
This guide explains how to perform analysis using SwiftMCPClient's
core functions.
...
```markdown
<!-- In SwiftMCPClient.md landing page -->
